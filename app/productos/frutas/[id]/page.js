...
return(
        <div className="bg-white relative h-full pb-200px">
            {/* <p className="text-2xl font-medium text-center font-serif bg-black text-white">Frutas</p>                 */}
            <Link href='/productos/frutas'>
            <div className="relative flex border-b-[1px] md:h-[7%]">
                <button className="p-[5px]"><IoIosArrowBack/></button>
                <p className="flex items-center text-[18px]">Frutas</p>
            </div>
            </Link>
            <div className="px-[2%]">
            {isLoading || !product  ? (
                ...
                <div>
                    ...
                    <img className="w-full h-[35vh] md:w-[75%] md:m-auto object-contain" src={product.img}></img>
                    ...
                    <div className="grid grid-cols-2 mt-[5%] px-[5%] pb-[10vh]">
                        <div className="grid-rows-6 grid-rows-[35px 35px 35px 35px 35px 35px] text-center grid border border-solid border-black rounded-t">
                            ...
                            <p className="border border-solid border-black p-1 text-[15px]">{product.quantity1 != null ? product.quantity1 : '-'}</p>
                            <p className="border border-solid border-black p-1 text-[15px]">{product.quantity2 != null ? product.quantity2 : '-'}</p>
                            <p className="border border-solid border-black p-1 text-[15px]">{product.quantity3 != null ? product.quantity3 : '-'}</p>
                            <p className="border border-solid border-black p-1 text-[15px]">{product.quantity4 != null ? product.quantity4 : '-'}</p>
                            <p className="border border-solid border-black p-1 text-[15px]">{product.quantity5 != null ? product.quantity5 : '-'}</p>
                        </div>
                        <div className=" grid-rows-6 grid-rows-[35px 35px 35px 35px 35px 35px] text-center grid border border-solid border-black rounded-t">
